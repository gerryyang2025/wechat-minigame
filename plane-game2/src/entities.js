...
var DOUBLE_SHOT_DURATION = 3;
var FIREPOWER_UPGRADE_DURATION = 10;
var BASE_FIRE_INTERVAL = 0.24;
var DOUBLE_SHOT_FIRE_INTERVAL = 0.16;
var FIREPOWER_UPGRADE_INTERVAL = 0.13;
var INK = '#342d28';
var SOFT_INK = '#6f6459';
...
function getAnimationFrame(frames, progress) {
  if (!frames || !frames.length) {
    return null;
  }

  var index = Math.floor(progress * frames.length);
...
Bullet.prototype.draw = function (ctx) {
  if (drawSprite(ctx, this.sprite, this.x, this.y, this.width, this.height)) {
    return;
  }
...
function EnemyBullet(game, sprite, x, y, pattern, options) {
  options = options || {};
  ...
  this.width = (this.pattern === 'heavy' ? 10 : (this.pattern === 'shard' ? 9 : 8)) * this.scale;
  this.height = (this.pattern === 'heavy' ? 18 : (this.pattern === 'shard' ? 18 : 16)) * this.scale;
...
EnemyBullet.prototype.draw = function (ctx) {
  ...
  if (this.sprite) {
    drawSprite(ctx, this.sprite, this.x, this.y, this.width, this.height, {
      rotate: this.angle
    });
    return;
  }
...
function Player(game, sprite, bulletSprite) {
  ...
  this.baseFireInterval = BASE_FIRE_INTERVAL;
...
Player.prototype.update = function (deltaTime) {
  ...
  this.fireInterval = this.hasFirepowerUpgrade()
    ...
    : (this.hasDoubleShot() ? DOUBLE_SHOT_FIRE_INTERVAL : this.baseFireInterval);
...
Player.prototype.draw = function (ctx) {
  var idleFrames = this.sprite && this.sprite.idle ? this.sprite.idle : null;
  var activeFrame = idleFrames && idleFrames.length
    ? idleFrames[Math.floor(this.animationTime * 10) % idleFrames.length]
    : null;
  ...
  if (!drawSprite(ctx, activeFrame, 0, 0, this.width, this.height)) {
...
Enemy.prototype.getSpriteFrame = function () {
  if (!this.sprite) {
    return null;
  }

  if (this.type === 'large') {
    if (this.health / this.maxHealth <= 0.34 && this.sprite.hit) {
      return this.sprite.hit;
    }

    if (this.health / this.maxHealth <= 0.68 && this.sprite.alt) {
      return this.sprite.alt;
    }
  }

  if (this.type === 'medium' && this.health < this.maxHealth && this.sprite.hit) {
    return this.sprite.hit;
  }

  return this.sprite.base || null;
};
...
Enemy.prototype.tryShoot = function (deltaTime) {
  ...
  for (var i = 0; i < volleyPattern.length; i += 1) {
    bullets.push(new EnemyBullet(
      this.game,
      this.game.assets && this.game.assets.bullets ? this.game.assets.bullets.enemy : null,
      volleyPattern[i].x,
      volleyPattern[i].y,
...
Enemy.prototype.draw = function (ctx) {
  ...
  var sprite = this.getSpriteFrame();
  ...
  ctx.translate(-this.width / 2, -this.height / 2);
  if (!drawSprite(ctx, sprite, 0, 0, this.width, this.height)) {
...
function Explosion(x, y, scale, frames, options) {
  options = options || {};
  this.x = x;
  this.y = y;
  this.scale = scale;
  this.frames = frames || null;
  this.width = options.width || (60 * scale);
  this.height = options.height || (60 * scale);
  this.age = 0;
  this.duration = options.duration || (this.frames && this.frames.length ? Math.max(0.2, this.frames.length * 0.08) : 0.28);
}
...
Explosion.prototype.draw = function (ctx) {
  if (this.frames && this.frames.length) {
    ...
    var frame = getAnimationFrame(this.frames, progress);
    ...
    if (frame) {
      drawSprite(
        ...
        this.width,
        this.height,
        { alpha: Math.max(0.72, 1 - progress * 0.18) }
      );
      return;
    }
  }
...
function PowerUp(game, type, x, y, sprite) {
  this.game = game;
  this.type = type;
...
PowerUp.prototype.draw = function (ctx) {
  ctx.save();
  ctx.translate(this.x, this.y);
  if (!drawSprite(ctx, this.sprite, 0, 0, this.size, this.size)) {
