...
MiniGameAudio.prototype.init = function () {
  ...
  this.bgm = this.createContext(this.getAssetPath('audio/game_music.mp3'), {
    loop: true,
    volume: 0.34
  });

  this.effectPools.button = this.createPool(this.getAssetPath('audio/button.mp3'), 2, 0.4, false);
  this.effectPools.bullet = this.createPool(this.getAssetPath('audio/bullet.mp3'), 4, 0.3, false);
  this.effectPools.enemy0Down = this.createPool(this.getAssetPath('audio/enemy0_down.mp3'), 3, 0.42, false);
  this.effectPools.enemy1Down = this.createPool(this.getAssetPath('audio/enemy1_down.mp3'), 2, 0.48, false);
  this.effectPools.enemy2Down = this.createPool(this.getAssetPath('audio/enemy2_down.mp3'), 1, 0.58, false);
  this.effectPools.hit = this.createPool(this.getAssetPath('audio/hit.wav'), 2, 0.46, false);
  this.effectPools.getBomb = this.createPool(this.getAssetPath('audio/get_bomb.mp3'), 1, 0.44, false);
  this.effectPools.getDoubleLaser = this.createPool(this.getAssetPath('audio/get_double_laser.mp3'), 1, 0.42, false);
  this.effectPools.useBomb = this.createPool(this.getAssetPath('audio/use_bomb.mp3'), 1, 0.5, false);
  this.effectPools.achievement = this.createPool(this.getAssetPath('audio/achievement.mp3'), 1, 0.42, false);
  this.effectPools.gameover = this.createPool(this.getAssetPath('audio/game_over.mp3'), 1, 0.55, false);
  this.effectPools.bossFly = this.createPool(this.getAssetPath('audio/big_spaceship_flying.mp3'), 1, 0.28, false);
};
